        ...
        try:
            ...
            if isinstance(result, list):
                ...
                for i, item in enumerate(result):
                    ...
                    # 扩展样品字段可能的名称
                    样品字段 = [
                        'testingSamplesNo', 'SampleName', 'sampleName', 'sampleNo', 'sampleCode',
                        'Manufacturer', 'manufacturer', 'producer', 'factoryName',
                        'TypeSpecification', 'typeSpecification', 'specification', 'model',
                        'sampleType', 'sampleStatus', 'batchNo', 'sampleDesc'
                    ]
                    sample_found = False
                    for key in 样品字段:
                        if key in item and item[key]:
                    ...
                    # 扩展报告字段可能的名称
                    报告字段 = [
                        'testingReportsNo', 'reportNo', 'reportCode', 'reportId',
                        'GenerationDate', 'generationDate', 'reportDate', 'issueDate',
                        'ReportProperties', 'reportProperties', 'reportType', 'reportStatus',
                        'Reviewer', 'reviewer', 'auditor', 'checkPerson',
                        'Approver', 'approver', 'confirmPerson',
                    ...
                    # 打印所有其他非空字段
                    print("\n== 其他信息 ==")
                    other_keys = [k for k in item.keys() 
                                if k not in 委托字段 
                                and k not in 样品字段 
                                and k not in 报告字段
                                and item[k]]  # 只显示非空字段
                ...
                # 提示用户可能需要查看详情
                if len(result) > 0 and 'testingOrderId' in result[0]:
                    print("\n提示: 要查看更详细的样品和报告信息,您可以在浏览器中打开以下链接:")
                    for i, item in enumerate(result):
                        if 'testingOrderId' in item:
                            detail_url = f"{base_url}/UI/IntegratedQueryManage/IntegratedDetail.aspx?testingOrderId={item['testingOrderId']}"
                            print(f"记录 {i+1} 详情链接: {detail_url}")
